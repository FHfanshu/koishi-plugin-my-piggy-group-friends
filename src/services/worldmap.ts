...
const MAP_URL_CANDIDATES = [
  'https://simplemaps.com/static/demos/resources/svg-library/svgs/world.svg',
  'https://cdn.amcharts.com/lib/4/geodata/svg/worldLow.svg',
]
...
export async function getWorldMapSvg(ctx: Context): Promise<string> {
  ...
  const dirPath = resolve(ctx.baseDir ?? process.cwd(), ...MAP_CACHE_RELATIVE_PATH.slice(0, -1))
  await fs.mkdir(dirPath, { recursive: true })

  let lastError: unknown = null
  for (const url of MAP_URL_CANDIDATES) {
    try {
      ...
      const svg = (response as any)?.data ?? response
      if (typeof svg === 'string' && svg.includes('<svg')) {
        await fs.writeFile(mapPath, svg, 'utf8')
        cachedWorldMapSvg = svg
        ctx.logger('pig').info(`World map SVG cached: ${mapPath}`)
        return svg
      }
      lastError = new Error('Invalid SVG content')
    } catch (e) {
      lastError = e
    }
  }

  throw new Error(`Failed to download world map SVG: ${lastError}`)
}
  ...
  for (const log of logs) {
    ...
    if (!existing) {
      countryMap.set(iso, {
        ...
        countryNameZh: log.countryZh || log.country,
        visitCount: 1,
...
export async function generateWorldMapCard(
  ...
  const processedSvg = processMapSvg(svg, data.visitedCountries)
...
<head>
  ...
  <style>
    ...
    * {
    ...
    .map-wrapper path.visited-2 { fill: #88BDBC !important; }
    .map-wrapper path.visited-3 { fill: #4F9D9D !important; }
    .map-wrapper path.visited-4 { fill: #254E58 !important; }
  ...
  <div class="container">
    ...
    <div class="map-section">
      <div class="map-header">
        ...
        <div class="map-note">Projection: Mercator</div>
      </div>
